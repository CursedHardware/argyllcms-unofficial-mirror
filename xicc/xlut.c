...

	{
		...
		/* we are doing an Output or Display device */
		{
			switch (h->colorSpace) {
				...
				default:
					xicp->e.c = 1;
					sprintf(xicp->e.m,"set_icxLuLut: can't handle color space %s",
...
//						bpop.p[0] = 0.0;
			...
			if (nw == 0) {
				xicp->e.c = 1;
				sprintf(xicp->e.m,"set_icxLuLut: can't handle test points without a white patch");
				p->del((icxLuBase *)p);
				return NULL;
			}
...
//			}
	...
	/* Setup and then create xfit object that does most of the work */
	{
		...
		if ((xf = new_xfit(icco)) == NULL) {
			p->pp->e.c = 2;
			sprintf(p->pp->e.m,"Creation of xfit object failed");
			p->del((icxLuBase *)p);
			return NULL;
		}
